...
impl Connection {
    fn parse_segments(&self, segments: &[u8]) -> Result<Digit> {
        let mut digit = 0;
        for segment in segments {
            digit |= 1 << self.segments[(segment - b'a') as usize] as i32;
        }

        match digit {
            0b1110111 => Ok(Digit::Zero),
            0b0010010 => Ok(Digit::One),
            0b1011101 => Ok(Digit::Two),
            0b1011011 => Ok(Digit::Three),
            0b0111010 => Ok(Digit::Four),
            0b1101011 => Ok(Digit::Five),
            0b1101111 => Ok(Digit::Six),
            0b1010010 => Ok(Digit::Seven),
            0b1111111 => Ok(Digit::Eight),
            0b1111011 => Ok(Digit::Nine),
            _ => bail!("Invalid digit"),
        }
    }
}

fn all_connections() -> impl Iterator<Item = Connection> {
    [
        Segment::A,
        Segment::B,
        Segment::C,
    ...
    .cloned()
    .permutations(7)
    .map(|segments| Connection { segments })
}

fn validate_connection<'a, I: IntoIterator<Item = &'a [u8]>>(
    connection: &Connection,
    segments: I,
) -> bool {
    let mut digits = [false; 10];
    for segment in segments {
        match connection.parse_segments(segment) {
            Ok(digit) => digits[digit as usize] = true,
            Err(_) => return false,
        }
    }
    digits.iter().all(|b| *b)
...
fn decide_connection<'a, I: IntoIterator<Item = &'a [u8]> + Clone>(segments: I) -> Connection {
    all_connections()
        .find(|connection| validate_connection(connection, segments.clone()))
        .unwrap()
}

const INPUT: &[u8] =
    b"eadbcf faceb faecgd gdefabc adc ad adbf gfacbe bceda dcegb | gdfcae adc cedbfa dafb
...
fn main() {
    ...
    for line in INPUT.split(|v| *v == b'\n') {
        let mut line = line.split(|v| *v == b'|');
        let (input, output) = (line.next().unwrap(), line.next().unwrap());

        let connection = decide_connection(input.split(|v| *v == b' ').take(10));

        for (i, output) in output.split(|v| *v == b' ').skip(1).enumerate() {
            let digit = connection.parse_segments(output).unwrap();
            result += digit as i32 * 10_u32.pow(3 - i as u32) as i32;
        }
    }
